...
public class INVOKEDYNAMIC extends Instruction {
  ...
  public Instruction execute(ThreadInfo ti) {
    ...
    try {
      ...
      } else if (bmi.getBmType() == BootstrapMethodInfo.BMType.SWITCH) {
        return executeSwitch(ti, bmi);
      }
  ...
  /**
   * Executes java.lang.runtime.SwitchBootstraps.typeSwitch/enumSwitch call sites,
   * which back Java 17+ pattern matching switches (JEP 441).
   *
   * Stack: ..., receiver, restartIndex => ..., matchIndex
   *
   * Semantics (matching the JDK implementation):
   *   - null receiver               -> -1
   *   - first matching label at or after restartIndex -> its index
   *   - no match                    -> labels.length
   *
   * The generated bytecode following this instruction (TABLESWITCH/LOOKUPSWITCH)
   * dispatches on the pushed index.
   */
  private Instruction executeSwitch(ThreadInfo ti, BootstrapMethodInfo bmi) {
    StackFrame frame = ti.getModifiableTopFrame();

    int restartIndex = frame.pop();
    int receiverRef = frame.pop();

    String[] labels = bmi.getSwitchLabels();
    if (labels == null) {
      labels = new String[0];
    }

    // null never matches a type label
    if (receiverRef == MJIEnv.NULL) {
      frame.push(-1);
      return getNext();
    }

    ElementInfo ei = ti.getHeap().get(receiverRef);
    if (ei == null) {
      frame.push(-1);
      return getNext();
    }

    int startIdx = (restartIndex > 0) ? restartIndex : 0;
    int matchIndex = labels.length;

    for (int i = startIdx; i < labels.length; i++) {
      if (matchesLabel(ti, ei, labels[i], bmi.isEnumSwitch())) {
        matchIndex = i;
        break;
      }
    }

    frame.push(matchIndex);
    return getNext();
  }

  /**
   * checks whether the receiver matches a single switch label. For type switches
   * this is an instanceof test against the label class, for enum switches the
   * label is the name of an enum constant.
   */
  private boolean matchesLabel (ThreadInfo ti, ElementInfo ei, String label, boolean isEnumSwitch) {
    if (label == null) {
      return false;
    }
    ...
    if (isEnumSwitch) {
      // labels of an enum switch are the names of enum constants
      int nameRef = ei.getReferenceField("name");
      if (nameRef != MJIEnv.NULL) {
        ElementInfo nameEi = ti.getHeap().get(nameRef);
        if (nameEi != null && label.equals(nameEi.asString())) {
          return true;
        }
      }

      // an unqualified label can only be a constant name, so there is no type
      // to test against - qualified labels fall through to the instanceof test
      if (label.indexOf('.') < 0) {
        return false;
      }
    }

    // a label that cannot be resolved simply does not match - it must not
    // abort the verification
    try {
      return ci.isInstanceOf(label);
    } catch (Throwable t) {
      return false;
    }
  }
